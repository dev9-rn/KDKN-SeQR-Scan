...
export default class InstituteCertificateAndPrint extends React.Component {
        ...
        await AsyncStorage.multiGet(['USERDATA', 'CERTIFICATESCANNEDDATA'], (err, result) => {		// USERDATA is set on InstituteLogin screen
            ...
            if (lData) {
                if (lData.scan_result == '1') {
                    this.setState({
                        serialNo: lData.serialNo,
                        certificateURI: lData.fileUrl,
    ...
    _showHeader() {
        if (Platform.OS == 'ios') {
            return (
                <Header style={{ backgroundColor: '#3e3e3e' }} hasTabs>
                    <Grid>
            ...
            <View style={{ flex: 1 }}>
                ...
                <View style={{ flex: 1, }}>
                    <Tabs tabBarUnderlineStyle={{ backgroundColor: 'white' }}  >
                        <Tab heading="View Certificate" tabStyle={{ backgroundColor: '#3e3e3e' }} textStyle={{ color: '#fff' }}
                            activeTabStyle={{ backgroundColor: '#3e3e3e', }} activeTextStyle={{ color: 'white', fontWeight: 'bold' }}>

                            <ViewCertificate props={this.props}
                                navigation={this.props.navigation} />
                        </Tab>
                        <Tab heading="View Printing Details" tabStyle={{ backgroundColor: '#3e3e3e' }} textStyle={{ color: '#fff' }} activeTabStyle={{ backgroundColor: '#3e3e3e', borderBottomColor: 'red' }} activeTextStyle={{ color: 'white', fontWeight: 'bold' }}>
